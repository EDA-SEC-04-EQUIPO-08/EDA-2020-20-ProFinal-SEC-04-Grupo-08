...
def max(heap):
    """
    Retorna el mayor elemento del heap, es decir el mayor elemento
    Args:
        heap: El arreglo con la informacion
    Returns:
      El mayor elemento del heap
    Raises:
        Exception
    """
    try:
...
def delMax(heap):
    """
    Retorna el mayor elemento del heap y lo elimina.
    Se reemplaza con el último elemento y se hace sink.
    Args:
        heap: El arreglo con la informacion
    Returns:
        El mayor elemento eliminado
    Raises:
        Exception
    """
    try:
...
def smaller(heap, element1, element2):
    ...
    """
    try:
        cmp = heap['cmpfunction'](element1[0], element2[0])
        if cmp < 0:
            return True
        return False
    except Exception as exp:
        error.reraise(exp, 'heap:smaller')
